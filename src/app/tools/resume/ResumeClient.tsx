...
const DRAFT_KEY = 'roots_tool_resume_draft';
...
export default function ResumePage() {
  ...
  if (initialState.current === null) {
      ...
      if (raw) {
        ...
        if (parsed && parsed.form) {
          initialState.current = {
            ...
            education: parsed.education?.length ? parsed.education : [emptyEntry()],
            work: parsed.work?.length ? parsed.work : [emptyEntry()],
            licenses: parsed.licenses?.length ? parsed.licenses : [emptyEntry()],
    ...
    // We'll set hasDraft in useEffect to avoid hydration mismatch
  }

  const [form, setForm] = useState<FormData>(initialState.current!.form);
  const [education, setEducation] = useState<TimeEntry[]>(initialState.current!.education);
  const [work, setWork] = useState<TimeEntry[]>(initialState.current!.work);
  const [licenses, setLicenses] = useState<TimeEntry[]>(initialState.current!.licenses);
  const [photoUrl, setPhotoUrl] = useState<string | null>(initialState.current!.photoUrl);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      try {
        const data = JSON.stringify({ form, education, work, licenses, photoUrl });
  ...
  }, [form, education, work, licenses, photoUrl]);
          ...
          <div className="space-y-6 max-w-2xl">
            ...
            {hasDraft && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 flex items-center justify-between text-sm">
                ...
                <button onClick={clearDraft} className="text-amber-600 hover:text-amber-800 text-xs underline">下書きを削除</button>
              </div>
            )}
            {/* Section 1: 基本情報 */}
            <section className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">

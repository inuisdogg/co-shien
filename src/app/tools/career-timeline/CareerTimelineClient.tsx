...
const TIMELINE_DRAFT_KEY = 'roots_tool_timeline_draft';
...
export default function CareerTimelinePage() {
  ...
  const initialState = useRef<{ entries: CareerEntry[]; userName: string } | null>(null);

  if (initialState.current === null) {
    try {
      const raw = typeof window !== 'undefined' ? localStorage.getItem(TIMELINE_DRAFT_KEY) : null;
      if (raw) {
        ...
        if (parsed && Array.isArray(parsed.entries)) {
          ...
          initialState.current = { entries: [], userName: '' };
        }
      } else {
        initialState.current = { entries: [], userName: '' };
      }
  ...
  const [entries, setEntries] = useState<CareerEntry[]>(initialState.current!.entries);
  const [showModal, setShowModal] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<Omit<CareerEntry, 'id'>>(emptyEntry());
  const [userName, setUserName] = useState(initialState.current!.userName);
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(TIMELINE_DRAFT_KEY);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      try {
        localStorage.setItem(TIMELINE_DRAFT_KEY, JSON.stringify({ entries, userName }));
      } catch {

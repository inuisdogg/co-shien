...
const CV_DRAFT_KEY = 'roots_tool_cv_draft';
...
export default function CvGeneratorPage() {
  ...
  const initialFormData = useRef<CvFormData | null>(null);
  ...
  if (initialFormData.current === null) {
    ...
    try {
      const raw = typeof window !== 'undefined' ? localStorage.getItem(CV_DRAFT_KEY) : null;
      if (raw) {
        ...
        if (parsed && typeof parsed.name === 'string') {
          initialFormData.current = {
            ...defaultData,
            ...parsed,
            work_entries: parsed.work_entries?.length ? parsed.work_entries : [createEmptyWork()],
            qualifications: parsed.qualifications?.length ? parsed.qualifications : [createEmptyQualification()],
          };
        } else {
          initialFormData.current = defaultData;
        }
      } else {
        initialFormData.current = defaultData;
      }
  ...
  const [formData, setFormData] = useState<CvFormData>(initialFormData.current!);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      try {
        localStorage.setItem(CV_DRAFT_KEY, JSON.stringify(formData));
      } catch {

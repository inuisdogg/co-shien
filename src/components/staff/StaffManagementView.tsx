...
const StaffManagementView: React.FC = () => {
  ...
  const handleOpenEdit = async (staff: Staff) => {
    ...
    const editData = {
      ...
      gender: (staff.gender || '') as '男性' | '女性' | 'その他' | '',
      postalCode: staff.postalCode || '',
      myNumber: staff.myNumber || '',
      hasSpouse: staff.hasSpouse || false,
      spouseName: staff.spouseName || '',
      basicPensionSymbol: staff.basicPensionSymbol || '',
      basicPensionNumber: staff.basicPensionNumber || '',
      employmentInsuranceStatus: (staff.employmentInsuranceStatus || 'joined') as 'joined' | 'not_joined' | 'first_time',
      employmentInsuranceNumber: staff.employmentInsuranceNumber || '',
      previousRetirementDate: staff.previousRetirementDate || '',
      previousName: staff.previousName || '',
      socialInsuranceStatus: (staff.socialInsuranceStatus || 'joined') as 'joined' | 'not_joined',
      hasDependents: staff.hasDependents || false,
      dependentCount: staff.dependentCount || 0,
      dependents: (staff.dependents || []).map((d: { name?: string; relationship?: string; birthDate?: string; myNumber?: string; id?: string; furigana?: string; gender?: string; occupation?: string; annualIncome?: string; notWorking?: boolean; notWorkingReason?: string; separateAddress?: string }) => ({
        id: d.id || `dep-${Date.now()}-${Math.random().toString(36).substring(7)}`,
        name: d.name || '',
        furigana: d.furigana || '',
        relationship: d.relationship || '',
        birthDate: d.birthDate || '',
        gender: (d.gender === 'male' || d.gender === 'female' ? d.gender : 'male') as 'male' | 'female',
        occupation: d.occupation || '',
        annualIncome: d.annualIncome || '',
        notWorking: d.notWorking || false,
        notWorkingReason: d.notWorkingReason || '',
        myNumber: d.myNumber || '',
        separateAddress: d.separateAddress || '',
      })),

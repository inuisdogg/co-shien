...
export const useFacilityData = () => {
  ...
  const [facilitySettings, setFacilitySettings] = useState<FacilitySettings>({
    ...
    capacity: {
      AM: 0, // 未設定時は0（timeSlots設定後に更新される）
      PM: 0,
    },
      ...
      try {
        ...
        if (data) {
          const timeSlotsData: FacilityTimeSlot[] = data.map((row) => ({
            ...
            capacity: row.capacity ?? 0, // 未設定時は0
  ...
  useEffect(() => {
    ...
    const fetchUsageRecords = async () => {
      try {
        console.log('📊 fetchUsageRecords: 利用実績データを取得中... facilityId:', facilityId);
        const { data, error } = await supabase
          .from('usage_records')
          .select('*')
          .eq('facility_id', facilityId)
        ...
        console.log('✅ fetchUsageRecords: 利用実績データ取得成功:', data?.length || 0, '件');
        if (data) {

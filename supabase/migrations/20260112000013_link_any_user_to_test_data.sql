-- ログインしているユーザー（ID: 66937dee-0b4f-4d2b-b075-63656b21a94a）をテストデータと紐付け
...
BEGIN
  -- ユーザーを取得（IDで検索）
  SELECT id, email INTO v_user_id, v_user_email FROM users WHERE id = '66937dee-0b4f-4d2b-b075-63656b21a94a';
  
  IF v_user_id IS NULL THEN
    RAISE NOTICE 'ユーザーID 66937dee-0b4f-4d2b-b075-63656b21a94a が見つかりません。メールアドレスで検索します。';
    -- メールアドレスで検索（14b0354@gmail.com以外の可能性もある）
    SELECT id, email INTO v_user_id, v_user_email FROM users WHERE email LIKE '%@%' AND user_type = 'client' ORDER BY created_at DESC LIMIT 1;
    
    IF v_user_id IS NULL THEN
      RAISE NOTICE '利用者アカウントが見つかりません';
      RETURN;
    END IF;
  END IF;

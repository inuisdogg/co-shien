...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='staff_documents' AND policyname='staff_documents_select') THEN
    CREATE POLICY staff_documents_select ON staff_documents FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='staff_documents' AND policyname='staff_documents_insert') THEN
    CREATE POLICY staff_documents_insert ON staff_documents FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='staff_documents' AND policyname='staff_documents_update') THEN
    CREATE POLICY staff_documents_update ON staff_documents FOR UPDATE USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='staff_documents' AND policyname='staff_documents_delete') THEN

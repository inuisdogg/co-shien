...
| # | チェック内容 | 対象ファイル |
|---|-------------|-------------|
| R4-1 | 時間枠×児童の予約表示整合性 | ScheduleView |
...
- `facility_addition_settings`
- `addition_staff_requirements`
...
| R5-6 | 地域区分×単価計算の整合性 | regional_units × base_rewards |
...
**対象:**
- チャット（chat_messages）
- コネクト（connect_meetings）
- ナレッジ（knowledge_articles）
- 研修記録（training_records）
- 書類管理（document_uploads）
